...
sidebar_custom_props:
  icon: Layers
  badge: Draft
description: "Proposed LLM Gateway Management API v1: apps & provider attachments, with typed inputs and examples."
---
...
Names are trimmed and compared case-insensitively. New names must be unique among enabled and disabled apps. A disabled app continues to reserve its name. If multiple legacy apps match a name, the proposed API returns `409 ambiguous_app_name` instead of choosing one silently.
...
| Input | Rule |
| --- | --- |
| `name` | Required display name, 1-200 characters in the draft schema. |
...
| Configuration sections | See [full configuration input](./configuration). Omitted controls begin unset in this normalized draft, with runtime defaults advertised by capabilities. |
