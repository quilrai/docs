...
**Google Workspace** is a QuilrAI-built MCP available in the MCP Store. It exposes Gmail, Google Calendar, and Google Drive tools. The MCP is multi-tenant and does not ship a shared Google OAuth client, so each connection must use its own Google OAuth Client ID and Client Secret created in Google Cloud.
...
| Export Google files | `export_google_drive_file` | Read only |

Delete-event tools are intentionally not included. `send_gmail_message` is the only destructive tool.

## Required OAuth Scopes

Add all of these to the OAuth consent screen. The authorization flow requests the complete set. The MCP requires `openid` and `userinfo.email` to accept a token, while individual Gmail, Calendar, and Drive tools return a missing-scope error if their workload scope was not granted.
...
| `https://www.googleapis.com/auth/drive.metadata.readonly` | Search Drive and read file and shared-drive metadata. Restricted scope. |
| `https://www.googleapis.com/auth/drive.readonly` | Find files and shared drives, read file metadata, and export Google Workspace files. Restricted scope. |
...
2. Open **APIs & Services** > **Library** and enable the **Gmail API**, **Google Calendar API**, and **Google Drive API**.
...
4. Add the eight scopes from [Required OAuth Scopes](#required-oauth-scopes).
...
- **Restricted-scope verification.** The Gmail and Drive read scopes are restricted, and the Calendar scopes are sensitive. An **External** app must complete Google's OAuth verification, including any security assessment Google requires, before it can serve users outside its test-user list. Until then, add users under **Test users** on the consent screen to authorize without verification. An **Internal** Workspace app does not need verification.
...
| `invalid_scope` or `403` from Gmail, Calendar, or Drive | The consent screen is missing a workload scope, or an API is not enabled. | Confirm all three APIs are enabled and all eight scopes are present, then reconnect. |
